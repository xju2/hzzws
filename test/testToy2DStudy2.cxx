...
int main(int argc, char** argv)
{
    ...
    for(int itoy = 0; itoy < ntoys; itoy++)
    {
        ...
        auto* toy_data = (RooDataSet*) toyws->data(Form("toyData%d", itoy));
        if (!toy_data) {
            cout << itoy << " does not exist"<< endl;
        }
        cout << "mG is constant: " << mH->isConstant() << endl;
        cout << "xs is constant: " << mu->isConstant() << endl;
        cout << "kappa is constant: " << kappa->isConstant() << endl;
        ...
        mu->setVal(mu_value);
        mu->setConstant(1);
        auto* toy_cond_nll = RooStatsHelper::createNLL(toy_data, mc);
        RooStatsHelper::minimize(toy_cond_nll, workspace);
        double cond_nll = toy_cond_nll->getVal();
        ...
        cout << itoy<< ", test_statistic: " << test_statistic << endl;
