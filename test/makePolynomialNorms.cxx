  ...
  for (int p(0);p<prod.size();++p){
    ...
    for (std::map<float,std::string>::iterator it=fileMap.begin(); it!= fileMap.end(); ++it){
      ...
      for (int i(0);i<tree->GetEntries();++i){
        ...
        std::string cat;
        /*
           if (prod_type==0 && event_type==0) { cat="ggF_4mu";  }
           if (prod_type==0 && event_type==1) { cat="ggF_4e";  }
           if (prod_type==0 && event_type==2) { cat="ggF_2mu2e"; }
           if (prod_type==0 && event_type==3) { cat="ggF_2e2mu"; }
           if (prod_type==1) cat="VBF";
           if (prod_type==2) cat="VH_Lep";
           if (prod_type==3) cat="VH_Had";
           */
    ...
    for (int c(0);c<categories.size();++c){
      ...
      can.Print(Form("$PWD/plots/%s/acceptance_%s_%s_%s.png",prod[p].c_str(),prod[p].c_str(),cat.c_str(),pol)); //TODO this directory needs to exist if you want the output plots
      can.Print(Form("$PWD/plots/%s/acceptance_%s_%s_%s.eps",prod[p].c_str(),prod[p].c_str(),cat.c_str(),pol)); //TODO this directory needs to exist if you want the output plots
    ...
    can.Update();
    can.Print(Form("$PWD/plots/%s/acceptance_%s_%s.png",prod[p].c_str(),prod[p].c_str(),pol)); //TODO this directory needs to exist if you want the output plots
    can.Print(Form("$PWD/plots/%s/acceptance_%s_%s.eps",prod[p].c_str(),prod[p].c_str(),pol)); //TODO this directory needs to exist if you want the output plots
