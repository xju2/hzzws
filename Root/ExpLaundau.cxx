...
ExpLaundau::ExpLaundau(const char* _name, 
    ...
    , workspace(new RooWorkspace("ExpLaundau"))
    , doSys(_doSys)
{

    Helper::readConfig(Form("%s/%s", _path, _input), '=', para_dic);
    setNormSysDicFromText(Form("%s/%s", _path, _norm_sys));
    shape_sys_names_ = new vector<string>();
}

ExpLaundau::~ExpLaundau()
{
    delete workspace;
    if(shape_sys_names_) delete shape_sys_names_;
}

bool ExpLaundau::setChannel(const RooArgSet& _obs, const char* _ch_name, bool with_sys)
{
...
bool ExpLaundau::addShapeSys(const TString& npName)
{
...
RooProduct* ExpLaundau::variable(const string& parname)
{
    string val_str;
    try {
...
FlexibleInterpVar* ExpLaundau::flexibleInterpVar(const string& fivName, vector<string>& names, 
