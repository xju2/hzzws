...
RooAbsPdf* AnalyticHMBkg::getPDF(){
  ...
  //Defining the pdf
  //
  auto x0 = new RooConstVar("x0","x0",240);
  //f1(x)
  const char * f1form = "(0.5+0.5*TMath::Erf((@0-@1)/@2))*((@4)/(1.0+TMath::Exp((@0-@1)/@3)))";
  auto f1 = new RooFormulaVar(Form("%s_%s_f1",pdf_name_.c_str(),category_name_.c_str()),f1form,     RooArgList(*x ,*m_var["a1"],*m_var["a2"],*m_var["a3"],*m_var["a4"]));
  auto f1x0 = new RooFormulaVar(Form("%s_%s_f1x0",pdf_name_.c_str(),category_name_.c_str()),f1form, RooArgList(*x0,*m_var["a1"],*m_var["a2"],*m_var["a3"],*m_var["a4"]));
  //f2(x)
