...
int main(int argc, char* argv[]){
    ...
    for (const auto& key : all_dic){
        ...
        TFile* file = TFile::Open(Form("%s_shape.root",sampleName.c_str()), "recreate");
        ...
        for(int i = 0; i < n_bins; i++){
            ...
            h_pdf_down->Fill(m4l, pdf_var["down"]);
            h_pdf_up->Fill(m4l, pdf_var["up"]);
            h_qcd_down->Fill(m4l, qcd_var["down"]);
            h_qcd_up->Fill(m4l, qcd_var["up"]);
        }
        string pdf_name = sample_dic.at("pdfName");
        string qcd_name = sample_dic.at("qcdName");
        for (auto& category : categories){
            TH1F* h_pdf_up_copy = (TH1F*) h_pdf_up->Clone(Form("%s-%s-up", pdf_name.c_str(), category.c_str()));
            TH1F* h_pdf_down_copy = (TH1F*) h_pdf_down->Clone(Form("%s-%s-down", pdf_name.c_str(), category.c_str()));
            TH1F* h_qcd_up_copy = (TH1F*) h_qcd_up->Clone(Form("%s-%s-up", qcd_name.c_str(), category.c_str()));
            TH1F* h_qcd_down_copy = (TH1F*) h_qcd_down->Clone(Form("%s-%s-down", qcd_name.c_str(), category.c_str()));
            h_pdf_up_copy->Write();
